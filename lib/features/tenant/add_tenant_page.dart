...
void _showTenantRequestDetails(BuildContext context, Map r) {
  final start = DateTime.tryParse('${r['leaseStartDate']}');
  final end = DateTime.tryParse('${r['leaseEndDate']}');
  showDialog(
    context: context,
    builder: (dialogContext) => AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          _DetailRow('Phone', '${r['tenantPhone']}'),
          _DetailRow('Email', '${r['tenantEmail']}'),
          _DetailRow('Lease',
              '${start != null ? _dateFmt.format(start) : '—'} — ${end != null ? _dateFmt.format(end) : '—'}'),
          _DetailRow('Rent/month', 'Rs ${r['rentPerMonth']}'),
          if ((r['depositAmount'] as num?) != null &&
              (r['depositAmount'] as num) > 0)
            _DetailRow('Deposit', 'Rs ${r['depositAmount']}'),
          if (r['status'] == 'Rejected' && r['rejectionReason'] != null)
            _DetailRow('Reason', '${r['rejectionReason']}'),
        ],
      ),
      actions: [
        TextButton(
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await submitTenantRequest(dio, {
        ...
        'leaseStartDate': DateTime.utc(
                _leaseStart!.year, _leaseStart!.month, _leaseStart!.day)
            .toIso8601String(),
        'leaseEndDate':
            DateTime.utc(_leaseEnd!.year, _leaseEnd!.month, _leaseEnd!.day)
                .toIso8601String(),
        'rentPerMonth': rent,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Tenant request submitted for admin approval')),
        );
      }
      await _load();
          ...
          : ListView(
              ...
              children: [
                  ...
                  ..._requests.map((raw) {
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        onTap: () => _showTenantRequestDetails(context, r),
                      ),
                    );
                  }),
                  const SizedBox(height: 24),
                ],
                const Text('Add a new tenant',
                    style:
                        TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
                ...
                TextField(
                    ...
                    decoration:
                        const InputDecoration(labelText: 'Tenant email')),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(_leaseStart == null
                            ? 'Lease start date'
                            : _leaseStart!.toIso8601String().split('T').first),
                      ),
                    ),
                    ...
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(_leaseEnd == null
                            ? 'Lease end date'
                            : _leaseEnd!.toIso8601String().split('T').first),
                      ),
                    ),
                  ],
                ),
