...
class _Stats {
  ...
  const _Stats(
      this.members, this.openComplaints, this.visitorsToday, this.collectedPct);
}
...
class _AdminShellState extends State<AdminShell> {
  ...
  Future<_Stats> _load(Dio dio) async {
    final results = await Future.wait([
      dio.get('/auth/members'),
      dio.get('/complaints'),
      dio.get('/visitors'),
      dio.get('/bills')
    ]);
    final members = (results[0].data['members'] as List).length;
    final complaints = (results[1].data['complaints'] as List);
    final visitors = (results[2].data['visitors'] as List);
    final bills = (results[3].data['bills'] as List);
    final openComplaints = complaints
        .where((c) => c['status'] == 'Open' || c['status'] == 'In progress')
        .length;
  ...
  Widget build(BuildContext context) {
    ...
    final activeProfile = auth is AuthAuthed
        ? ((auth.user['profiles'] as List?) ?? const [])
            .cast<Map?>()
            .firstWhere(
              (p) =>
                  p != null &&
                  '${p['_id']}' == '${auth.claims['activeProfileId']}',
              orElse: () => null,
            )
        : null;
    final societyName =
        activeProfile?['societyName']?.toString() ?? 'Admin console';
    return Scaffold(
      body: SafeArea(
            ...
            child: Column(
              ...
              children: [
                ...
                _action(
                    ...
                    Icons.campaign,
                    'Post a notice',
                    'Announce to the whole society',
                    () => _postNotice(context, dio)),
                _action(
                    context,
                    Icons.receipt_long,
                    'Generate bill',
                    'Raise maintenance for a member',
                    () => _generateBill(context, dio)),
                _action(context, Icons.fact_check, 'Manage complaints',
                    'Review and resolve tickets', () {
                  Haptics.light();
                  context.push('/complaints');
                }),
                _action(context, Icons.notifications_outlined, 'Notifications',
                    'Society-wide alerts and SOS', () {
                  Haptics.light();
  ...
  Widget _action(BuildContext context, IconData icon, String title, String sub,
      VoidCallback onTap) {
    return PressEffect(
      onTap: onTap,
      child: Container(
        ...
        [
          ...
          TextField(
              ...
              decoration: const InputDecoration(
                  hintText: 'Title (at least 10 characters)')),
          const SizedBox(height: 12),
          TextField(
              controller: description,
              maxLines: 3,
              decoration: const InputDecoration(
                  hintText: 'Message (at least 30 characters)')),
        ],
        'Publish', () async {
      if (title.text.trim().length < 10) {
        return 'Title must be at least 10 characters';
      }
      if (description.text.trim().length < 30) {
    ...
    _sheet(
        ...
        [
          TextField(
              controller: flat,
              decoration:
                  const InputDecoration(hintText: 'Flat number, e.g. A-1203')),
          const SizedBox(height: 12),
          TextField(
              controller: amount,
      ...
      if (flat.text.trim().isEmpty || amt == null || amt <= 0) {
        return 'Enter a valid flat number and amount';
      }
      final members = (await dio.get('/auth/members')).data['members'] as List;
      final match = members.cast<Map?>().firstWhere(
            (m) =>
                '${m?['flatNo']}'.toLowerCase() ==
                flat.text.trim().toLowerCase(),
            orElse: () => null,
          );
  ...
  void _sheet(BuildContext context, String title, List<Widget> fields,
      String cta, Future<String?> Function() onSubmit) {
        ...
        return StatefulBuilder(
          builder: (sheetCtx, setState) => Padding(
            padding: EdgeInsets.fromLTRB(
                22, 18, 22, MediaQuery.of(sheetCtx).viewInsets.bottom + 24),
            child: Column(
                    ...
                    try {
                      ...
                      if (context.mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('$title done')));
                      }
