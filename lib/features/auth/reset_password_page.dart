...
class _ResetPasswordViewState extends State<_ResetPasswordView> {
  ...
  void _submit(BuildContext context) {
    if (_code.text.trim().length != 6) {
      Haptics.heavy();
      showPulseToast(context, 'Enter the 6-digit code from your email',
          kind: PulseToastKind.error);
      return;
    }
    if (_newPw.text.length < 6) {
      Haptics.heavy();
      showPulseToast(context, 'Password must be at least 6 characters',
          kind: PulseToastKind.error);
      return;
    }
    if (_newPw.text != _confirmPw.text) {
      Haptics.heavy();
      showPulseToast(context, 'Passwords do not match',
          kind: PulseToastKind.error);
      return;
    }
    context.read<PasswordResetBloc>().add(
          ResetPasswordRequested(
              widget.identifier, _code.text.trim(), _newPw.text),
        );
  }
                    ...
                    return Column(
                      ...
                      children: [
                        ...
                        PulseButton(
                          ...
                          loading: loading,
                          disabled: _code.text.trim().isEmpty ||
                              _newPw.text.isEmpty ||
                              _confirmPw.text.isEmpty,
