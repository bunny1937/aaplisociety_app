...
class _EmergencyContactPageState extends State<EmergencyContactPage> {
  ...
  Map? get _pending {
    for (final r in _requests) {
      final m = r as Map;
      if (m['section'] == 'EmergencyContact' && m['status'] == 'Pending') {
        return m;
      }
    }
    return null;
  ...
  Future<void> _edit() async {
    ...
    final phoneNumber =
        TextEditingController(text: current?['phoneNumber']?.toString() ?? '');
    final relation =
        TextEditingController(text: current?['relation']?.toString() ?? '');
    final address =
        TextEditingController(text: current?['address']?.toString() ?? '');
    final submitted = await showPulseSheet<bool>(
      context,
      title: 'Edit emergency contact',
      builder: (sheetCtx) => _EmergencyContactForm(
          name: name,
  ...
  Widget build(BuildContext context) {
    ...
    final pendingPayload = pending?['payload'] as Map?;
    final current = widget.member?['emergencyContact'] as Map?;
    String field(String key) =>
        (current?[key] ?? pendingPayload?[key])?.toString() ?? '';
    final isPending = pendingPayload != null;
    ...
    final body = ListView(
      ...
      children: [
        ProfileInfoRow(
            label: 'Name',
            value: field('name').isEmpty
                ? null
                : '${field('name')}${isPending && current?['name'] == null ? ' (pending)' : ''}'),
        ProfileInfoRow(
            label: 'Phone',
            value: field('phoneNumber').isEmpty
                ? null
                : '${field('phoneNumber')}${isPending && current?['phoneNumber'] == null ? ' (pending)' : ''}'),
        ProfileInfoRow(
            label: 'Relation',
            value: field('relation').isEmpty
                ? null
                : '${field('relation')}${isPending && current?['relation'] == null ? ' (pending)' : ''}'),
        ProfileInfoRow(
            label: 'Address',
            value: field('address').isEmpty ? null : field('address')),
        if (pending != null)
          PendingChangeNote(
            text: current == null
                ? 'Awaiting admin approval'
                : 'Change to ${pendingPayload?['name']} (${pendingPayload?['relation']}, ${pendingPayload?['phoneNumber']}) — pending approval',
          ),
        const SizedBox(height: 20),
        if (widget.canEdit)
          pending != null
              ? Text('An emergency contact change is awaiting admin approval',
                  style: TextStyle(
                      fontSize: 12.5,
                      color: t.fg4,
    ...
    return Scaffold(
        appBar: AppBar(title: const Text('Emergency contact')), body: body);
  }
}
    ...
    try {
      ...
      await submitProfileEditRequest(dio, {
        ...
        'payload': {
          ...
          if (widget.address.text.trim().isNotEmpty)
