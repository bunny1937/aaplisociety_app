...
class FlatDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final votingText = votingRights == null
        ? null
        : (votingRights == true ? 'Eligible' : 'Not eligible');
    final body = ListView(
      ...
      children: [
        ProfileInfoRow(
          label: 'Flat',
          value: flatNo != null
              ? '$flatNo${wing != null && wing!.isNotEmpty ? ' ($wing wing)' : ''}'
              : null,
        ),
        ProfileInfoRow(label: 'Type', value: member?['flatType']?.toString()),
        ProfileInfoRow(
            label: 'Ownership', value: member?['ownershipType']?.toString()),
    ...
    return Scaffold(
        appBar: AppBar(title: const Text('Flat details')), body: body);
  }
}
