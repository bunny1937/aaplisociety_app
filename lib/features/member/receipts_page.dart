    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: AsyncView<List>(
          fetch: () async =>
              (await dio.get('/receipts')).data['receipts'] as List,
          cacheKey: '/receipts',
            ...
            return CustomScrollView(
              ...
              slivers: [
                SliverToBoxAdapter(
                  child: PulseTopBar(
                      title: 'Receipts',
                      subtitle: 'Payment receipts for your flat',
                      leading: BackButton(color: t.fg2)),
                ),
                if (receipts.isEmpty)
                  const SliverToBoxAdapter(
                      child: PulseEmptyState(
                          illo: PulseIllo.noData, title: 'No receipts yet'))
                else
                  SliverPadding(
                    ...
                    sliver: SliverList.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        final amount = (r['amountReceived'] as num?) ??
                            (r['amount'] as num?) ?? 0;
                        final applied = (r['amountApplied'] as num?) ?? amount;
                        final advance =
                            (r['advanceCreditCreated'] as num?) ?? 0;
                        final remaining =
                            (r['remainingBalance'] as num?) ?? 0;
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Applied ${inr(applied)}${advance > 0 ? ' · Advance ${inr(advance)}' : ''} · Remaining ${inr(remaining)}',
                                      style: TextStyle(
                                          fontSize: 10.8, color: t.fg4),
                                    ),
                                  ],
                                ),
                              ),
                              Column(
                                ...
                                children: [
                                  ...
                                  GestureDetector(
                                    onTap: () async {
                                      final bytes = await renderReceiptPdf(r,
