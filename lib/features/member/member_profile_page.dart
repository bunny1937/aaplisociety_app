...
class _SosControlState extends State<_SosControl> {
  ...
  ///
  /// Deliberately AFTER the hold-to-confirm rather than before: the hold is the
  /// panic action and must stay the fastest possible path. This sheet is a
  /// couple of taps that make the alert useful, and it defaults to a valid note
  /// so hitting "Raise SOS now" immediately still works.
  ...
  Future<void> _raise() async {
    ...
    try {
      // `note` is persisted as the visitor's purposeNote server-side and is what
      // the guard's SOS card now reads.
