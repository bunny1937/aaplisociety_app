...
class _PulseIllustrationState extends State<PulseIllustration>
...
class _IllustrationPainter extends CustomPainter {
  ...
  _IllustrationPainter(
      {required this.kind, required this.tokens, required this.progress});
  @override
    ...
    switch (kind) {
      case PulseIllo.emptyInbox:
        ...
        final env = Rect.fromCenter(
            center: c, width: size.width * 0.5, height: size.width * 0.36);
        ...
        canvas.drawCircle(Offset(c.dx + r * 0.42, c.dy - r * 0.42), r * 0.14,
      ...
      case PulseIllo.celebrate:
        ...
        if (kind == PulseIllo.celebrate) {
          ...
          for (var i = 0; i < dots.length; i++) {
            canvas.drawCircle(c + Offset(dots[i].dx * r, dots[i].dy * r),
                r * 0.045, Paint()..color = colors[i]);
          }
        }
      case PulseIllo.gate:
        _bg(canvas, c, r, tokens.brandSoft);
        ...
        canvas.drawRRect(
            RRect.fromRectAndRadius(
                Rect.fromCenter(
                    center: c + Offset(-r * 0.4, 0), width: barW, height: barH),
                const Radius.circular(3)),
            Paint()..color = tokens.brand);
        canvas.drawRRect(
            RRect.fromRectAndRadius(
                Rect.fromCenter(
                    center: c + Offset(r * 0.4, 0), width: barW, height: barH),
        ...
        canvas.drawRRect(
            ...
            Paint()..color = tokens.accent);
        canvas.drawCircle(
            c + Offset(0, r * 0.12), r * 0.2, Paint()..color = tokens.surface);
        canvas.drawCircle(
            c + Offset(0, r * 0.12),
            r * 0.2,
            Paint()
      ...
      case PulseIllo.noData:
        ...
        final doc = Rect.fromCenter(
            center: c, width: size.width * 0.42, height: size.width * 0.38);
        canvas.drawRRect(RRect.fromRectAndRadius(doc, const Radius.circular(6)),
        ...
        for (var i = 0; i < 3; i++) {
          ...
          canvas.drawLine(Offset(doc.left + 6, y),
              Offset(doc.right - 6 - i * 8, y), linePaint);
        }
      case PulseIllo.shield:
        ...
        path.quadraticBezierTo(
            c.dx + w / 2, c.dy + h * 0.34, c.dx, c.dy + h / 2);
        path.quadraticBezierTo(
            c.dx - w / 2, c.dy + h * 0.34, c.dx - w / 2, c.dy - h * 0.02);
        path.lineTo(c.dx - w / 2, c.dy - h / 2 + h * 0.18);
        path.close();
        ...
        canvas.drawPath(
            ...
            Paint()
              ...
              ..strokeWidth = 2.5);
        _animatedCheck(
            canvas, c + Offset(0, r * 0.02), r * 0.22, tokens.brand, progress);
    }
  }

  void _bg(Canvas canvas, Offset c, double r, Color color) =>
      canvas.drawCircle(c, r, Paint()..color = color);
  void _stroke(
      Canvas canvas, Color color, double w, void Function(Path) build) {
    final p = Path();
    build(p);
    canvas.drawPath(
        p,
  ...
  void _animatedCheck(
      Canvas canvas, Offset c, double r, Color color, double progress) {
    final path = Path()
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(title,
              style: TextStyle(
                  fontSize: 15, fontWeight: FontWeight.w700, color: t.fg1)),
          if (subtitle != null) ...[
            ...
            SizedBox(
              ...
              child: Text(subtitle!,
                  ...
                  style: TextStyle(fontSize: 12.5, color: t.fg4, height: 1.5)),
            ),
          ],
          if (action != null)
