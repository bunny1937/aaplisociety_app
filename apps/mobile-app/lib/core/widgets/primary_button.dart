...
class PrimaryButton extends StatefulWidget {
  ...
  final IconData? icon;
  final bool ledger;
  const PrimaryButton({super.key, required this.label, this.onPressed, this.loading = false, this.icon, this.ledger = false});
  @override
...
class _PrimaryButtonState extends State<PrimaryButton> {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTap: disabled ? null : () { Haptics.medium(); widget.onPressed!(); },
      child: AnimatedScale(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: (widget.ledger ? AppColors.brass : AppColors.blue).withValues(alpha: disabled ? 0.0 : 0.3),
                blurRadius: 18,
          ...
          child: Center(
            child: widget.loading
                ? SizedBox(height: 22, width: 22, child: CircularProgressIndicator(strokeWidth: 2.4, color: labelColor))
                : Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (widget.icon != null) ...[Icon(widget.icon, color: labelColor, size: 20), const SizedBox(width: 8)],
                      Text(
                        widget.label,
                        style: widget.ledger
                            ? GoogleFonts.manrope(color: labelColor, fontSize: 16, fontWeight: FontWeight.w700)
