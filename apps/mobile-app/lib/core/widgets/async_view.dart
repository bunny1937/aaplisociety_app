...
class AsyncView<T> extends StatefulWidget {
  ...
  const AsyncView({super.key, required this.fetch, required this.builder, this.isEmpty, this.emptyBuilder, this.loadingBuilder, this.cacheKey});
...
class AsyncViewState<T> extends State<AsyncView<T>> {
  ...
  Future<T> _fetchWithCacheFallback() async {
    try {
      ...
      if (widget.cacheKey != null && OfflineCache.isReady) OfflineCache.write(widget.cacheKey!, result);
      _servingFromCache = false;
      return result;
    } catch (err) {
      if (widget.cacheKey != null && OfflineCache.isReady && OfflineCache.has(widget.cacheKey!)) {
    ...
    return FutureBuilder<T>(
      ...
      builder: (context, snap) {
        ...
        if (widget.isEmpty?.call(data) ?? false) {
          ...
          content = RefreshIndicator(onRefresh: reload, child: widget.builder(context, data));
        }
...
class _OfflineBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.cloud_off_rounded, size: 15, color: Colors.orange.shade800),
          const SizedBox(width: 8),
          Text('Offline — showing last saved data', style: TextStyle(fontSize: 12, color: Colors.orange.shade800, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
