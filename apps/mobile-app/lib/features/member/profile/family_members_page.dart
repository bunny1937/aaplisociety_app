...
class FamilyMembersPage extends StatefulWidget {
  ...
  final bool canEdit;
  const FamilyMembersPage({super.key, required this.members, required this.canEdit});
...
class _FamilyMembersPageState extends State<FamilyMembersPage> {
  ...
  List get _familyRequests => _requests.where((r) => (r as Map)['section'] == 'FamilyMember' && r['status'] == 'Pending').toList();

  Map? _pendingFor(String familyMemberId) {
    for (final r in _familyRequests) {
      final m = r as Map;
      if (m['action'] != 'Add' && '${m['familyMemberId']}' == familyMemberId) return m;
    }
    return null;
  }

  List get _pendingAdds => _familyRequests.where((r) => (r as Map)['action'] == 'Add').toList();

  Future<void> _openForm({Map? existing, required String action, String? familyMemberId}) async {
    ...
    final contactNumber = TextEditingController(text: existing?['contactNumber']?.toString() ?? '');
    final occupation = TextEditingController(text: existing?['occupation']?.toString() ?? '');
  ...
  Future<void> _remove(String familyMemberId, String label) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Text('This submits a request to remove $label — it takes effect once an admin approves it.'),
        actions: [
    ...
    try {
      final dio = context.read<Dio>();
      await submitProfileEditRequest(dio, {'section': 'FamilyMember', 'action': 'Remove', 'familyMemberId': familyMemberId});
      await _load();
          ...
          : ListView(
              ...
              children: [
                ...
                ...widget.members.map((m) {
                  ...
                  final sub = [m['relation'], m['age'] != null ? '${m['age']} yrs' : null]
                            ...
                            children: [
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text('${m['name'] ?? '—'}', style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700, color: t.fg1)),
                              ...
                              if (widget.canEdit && pending == null) ...[
                                IconButton(
                                  ...
                                  onPressed: () => _openForm(existing: m, action: 'Edit', familyMemberId: familyMemberId),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.delete_outline, size: 19, color: Colors.red),
                                  onPressed: () => _remove(familyMemberId, '${m['name']}'),
                                ),
                              ],
                            ],
                          ),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              ...
                              style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: t.fg1),
                            ),
                          ),
                        ],
                      ),
...
class _FamilyMemberFormState extends State<_FamilyMemberForm> {
  ...
  Future<void> _submit() async {
    ...
    if (contactNumber.isNotEmpty && !RegExp(r'^[0-9]{10}$').hasMatch(contactNumber)) {
      setState(() => _error = 'Enter a valid 10-digit phone number, or leave it blank');
      return;
    }
    ...
    try {
      ...
      final body = <String, dynamic>{
        ...
        'payload': {
          ...
          if (widget.occupation.text.trim().isNotEmpty) 'occupation': widget.occupation.text.trim(),
        },
      };
      if (widget.familyMemberId != null) body['familyMemberId'] = widget.familyMemberId;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        TextField(controller: widget.relation, decoration: const InputDecoration(labelText: 'Relation (e.g. Spouse, Child)')),
        const SizedBox(height: 12),
        TextField(controller: widget.age, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Age (optional)')),
        const SizedBox(height: 12),
        TextField(controller: widget.contactNumber, keyboardType: TextInputType.phone, decoration: const InputDecoration(labelText: 'Phone (optional, 10 digits)')),
        const SizedBox(height: 12),
        TextField(controller: widget.occupation, decoration: const InputDecoration(labelText: 'Occupation (optional)')),
        if (_error != null) ...[
