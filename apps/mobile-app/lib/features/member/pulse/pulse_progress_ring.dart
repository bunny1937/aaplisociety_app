...
class PulseProgressRing extends StatelessWidget {
  ...
  const PulseProgressRing({super.key, required this.pct, this.size = 96, this.stroke = 9, this.color, this.child});
        ...
        return SizedBox(
          ...
          child: Stack(
            ...
            children: [
              CustomPaint(
                size: Size(size, size),
                painter: _RingPainter(pct: value, stroke: stroke, track: t.surface3, color: color ?? t.brand),
              ),
              if (child != null) child!,
            ],
          ),
...
class _RingPainter extends CustomPainter {
  ...
  final Color track;
  final Color color;
  _RingPainter({required this.pct, required this.stroke, required this.track, required this.color});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: center, radius: r), -math.pi / 2, sweep, false, fgPaint);
  }
