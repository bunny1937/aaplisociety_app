...
const List<DocumentFieldConfig> tenantDocumentFields = [
  DocumentFieldConfig(field: 'contract', maxBytes: 1048576, allowedExtensions: ['pdf'], label: 'Lease contract (PDF)'),
  DocumentFieldConfig(field: 'signature', maxBytes: 524288, allowedExtensions: ['jpg', 'jpeg', 'png'], label: 'Tenant signature'),
  DocumentFieldConfig(field: 'aadhaar', maxBytes: 524288, allowedExtensions: ['jpg', 'jpeg', 'png'], label: 'Aadhaar card photo'),
  DocumentFieldConfig(field: 'policeVerification', maxBytes: 524288, allowedExtensions: ['jpg', 'jpeg', 'png'], label: 'Police verification photo'),
];

String _sizeLabel(int bytes) => bytes >= 1048576 ? '${(bytes / 1048576).round()}MB' : '${(bytes / 1024).round()}KB';
...
/// not a hard floor).
String? validateDocumentFile(DocumentFieldConfig config, int fileSizeBytes, String fileExtension) {
...
class DocumentUploadField extends StatefulWidget {
  ...
  const DocumentUploadField({super.key, required this.config, required this.onPicked});
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            border: Border.all(color: _error != null ? Colors.red : Colors.grey.shade400),
            borderRadius: BorderRadius.circular(12),
          ),
          child: Row(
            children: [
              Icon(_fileName != null ? Icons.check_circle : Icons.upload_file, color: _fileName != null ? Colors.green : Colors.grey),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(widget.config.label, style: const TextStyle(fontWeight: FontWeight.w600)),
                    if (_fileName != null) Text(_fileName!, style: const TextStyle(fontSize: 12, color: Colors.grey)),
                    if (_error != null) Text(_error!, style: const TextStyle(fontSize: 12, color: Colors.red)),
                  ],
                ),
              ),
            ],
