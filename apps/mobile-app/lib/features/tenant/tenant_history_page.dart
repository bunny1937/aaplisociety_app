...
class _TenantHistoryPageState extends State<TenantHistoryPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await submitTenantHistory(dio, {
        ...
        if (_tenantEmail.text.trim().isNotEmpty) 'tenantEmail': _tenantEmail.text.trim(),
        ...
        if (_moveOutReason.text.trim().isNotEmpty) 'moveOutReason': _moveOutReason.text.trim(),
      });
      _tenantName.clear();
      _tenantPhone.clear();
      _tenantEmail.clear();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tenant history saved')));
      }
      await _load();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Tenant History')),
      body: _loading
          ...
          : ListView(
              ...
              children: [
                if (_history.isNotEmpty) ...[
                  const Text('Past tenants', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
                ...
                const Text('Add a past tenant', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
                ...
                const SizedBox(height: 12),
                TextField(controller: _tenantEmail, keyboardType: TextInputType.emailAddress, decoration: const InputDecoration(labelText: 'Tenant email (optional)')),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(_startDate == null ? 'Start date' : dateFmt.format(_startDate!)),
                      ),
                    ),
                    ...
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(_endDate == null ? 'End date' : dateFmt.format(_endDate!)),
                      ),
                    ),
                  ],
                ),
                ...
                const SizedBox(height: 12),
                TextField(controller: _moveOutReason, decoration: const InputDecoration(labelText: 'Move-out reason (optional)')),
                if (_formError != null) ...[
