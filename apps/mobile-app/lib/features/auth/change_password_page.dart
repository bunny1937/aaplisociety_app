...
class ChangePasswordPage extends StatefulWidget {
  ...
  const ChangePasswordPage({super.key, this.forced = false, this.roleForRedirect});
  @override
...
class _ChangePasswordPageState extends State<ChangePasswordPage> {
  ...
  Future<void> _submit() async {
    if (_next.text.length < 6) {
      Haptics.heavy();
      showAppToast(context, 'Password must be at least 6 characters', kind: AppToastKind.alert);
      return;
    }
    ...
    try {
      ...
      Haptics.heavy();
      setState(() => _busy = false);
      showAppToast(context, apiErrorMessage(err, 'Could not update password'), kind: AppToastKind.alert);
    }
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      appBar: AppBar(
        ...
        title: Text('Change password', style: GoogleFonts.fraunces(fontWeight: FontWeight.w600, color: context.headline)),
      ),
      body: SafeArea(
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: AppColors.brass.withValues(alpha: 0.35), width: 1),
                ),
                child: Row(
                  children: [
                    const Icon(Icons.shield_outlined, color: AppColors.brass, size: 30),
                    ...
                    Expanded(
                      child: Text(
                        ...
                        style: GoogleFonts.manrope(color: AppColors.paper, height: 1.4),
                      ),
                    ),
                  ],
                ),
              ...
              PrimaryButton(label: 'Update password', icon: Icons.check, ledger: true, loading: _busy, onPressed: _submit),
            ],
          ),
        ),
      ),
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: IconButton(
          icon: Icon(_obscure ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: AppColors.brass),
