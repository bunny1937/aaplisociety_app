...
class PasswordResetBloc extends Bloc<PasswordResetEvent, PasswordResetState> {
  ...
  PasswordResetBloc(this.dio) : super(PasswordResetInitial()) {
    on<ForgotPasswordRequested>((e, emit) async {
      ...
      try {
        await dio.post('/auth/forgot-password', data: {'identifier': e.identifier});
        emit(PasswordResetCodeSent());
      } on DioException catch (err) {
        emit(PasswordResetError(apiErrorMessage(err, 'Could not send reset code')));
      }
    });
    on<ResetPasswordRequested>((e, emit) async {
      ...
      try {
        ...
        emit(PasswordResetError(apiErrorMessage(err, 'Could not reset password')));
      }
    });
  }
}
