...
class _ComplaintsPageState extends State<ComplaintsPage> {
  ...
  static const _categories = ['maintenance', 'water', 'parking', 'security', 'noise', 'other'];
  static const _categoryEmoji = {
    'maintenance': '🔧', 'water': '💧', 'parking': '🚗', 'security': '🔒', 'noise': '🔊', 'other': '📋',
  };
      ...
      builder: (sheetCtx) => StatefulBuilder(
        builder: (sheetCtx, setSheet) {
          ...
          return Column(
            ...
            children: [
              Text('Raised anonymously, reviewed by admin', style: TextStyle(fontSize: 12, color: t.fg4)),
              const SizedBox(height: 16),
              Text('Category', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: t.fg2)),
                ...
                children: _categories.map((c) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Text('${_categoryEmoji[c]} ${c[0].toUpperCase()}${c.substring(1)}', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: sel ? t.brand : t.fg3)),
                    ),
                  );
                }).toList(),
              ),
              const SizedBox(height: 16),
              Text('Describe the issue', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, color: t.fg2)),
              const SizedBox(height: 8),
              Container(
                ...
                child: TextField(
                  ...
                  decoration: const InputDecoration(hintText: 'What happened?', border: InputBorder.none, contentPadding: EdgeInsets.all(12)),
                ),
              ),
              if (error != null) ...[
                const SizedBox(height: 8),
                ...
                onTap: () async {
                  ...
                  try {
                    await sheetCtx.read<Dio>().post('/complaints', data: {
                      'category': cat,
                      'title': desc.text.trim().length > 120 ? desc.text.trim().substring(0, 120) : desc.text.trim(),
  ...
  Future<void> _detail(BuildContext context, Map it, bool canManage) async {
    ...
    final note = TextEditingController(text: it['resolutionNote']?.toString() ?? '');
    var submitting = false;
    String? error;
    ...
    await showPulseSheet(
      ...
      builder: (sheetCtx) => StatefulBuilder(
        builder: (sheetCtx, setSheet) {
          final t = sheetCtx.pulse;
          final stages = [
            TimelineStage(label: 'Raised', timestamp: _fmt(it['createdAt']), color: t.brand),
            if (status != 'Open')
              TimelineStage(label: status, timestamp: _fmt(it['updatedAt']), note: it['resolutionNote']?.toString(), color: t.fg2),
          ];
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(
                children: [
                  Text('${_categoryEmoji[it['category']] ?? '📋'} ${it['category']}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: t.fg4)),
                  const Spacer(),
                  PulsePill(label: status, tone: _statusTone(status)),
                ],
              ),
              const SizedBox(height: 6),
              Text('${it['description'] ?? ''}', style: TextStyle(fontSize: 13, color: t.fg3, height: 1.4)),
              const SizedBox(height: 18),
              LedgerTimeline(stages: stages),
              if (canManage) ...[
                const SizedBox(height: 6),
                Text('Update status', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: t.fg1)),
                const SizedBox(height: 10),
                PulseSegmented<String>(
                  value: status,
                  onChanged: (v) => setSheet(() => status = v),
                  options: statuses.map((s) => PulseSegmentedOption(value: s, label: s)).toList(),
                ),
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: const InputDecoration(hintText: 'Resolution note (optional)', border: InputBorder.none, contentPadding: EdgeInsets.all(12)),
                  ),
                ),
                if (error != null) ...[
                  const SizedBox(height: 8),
                ...
                PulseButton(
                  ...
                  onTap: () async {
                    setSheet(() { submitting = true; error = null; });
                    try {
                      await sheetCtx.read<Dio>().patch('/complaints/${it['_id']}/status', data: {'status': status, 'resolutionNote': note.text.trim()});
                      Haptics.success();
                      if (sheetCtx.mounted) Navigator.of(sheetCtx).pop();
                      await _listKey.currentState?.reload();
                    } on DioException catch (err) {
                      Haptics.heavy();
                      setSheet(() { submitting = false; error = apiErrorMessage(err); });
                    }
                  },
                ),
              ],
        ...
        child: CustomScrollView(
          slivers: [
            SliverToBoxAdapter(
              child: PulseTopBar(
                title: 'Complaints',
                subtitle: canManage ? 'All complaints' : 'Raised anonymously, reviewed by admin',
                trailing: [
                  if (!canManage)
                    PulseButton(label: '+ New', size: PulseBtnSize.sm, onTap: () => _openNew(context)),
                ],
              ),
            ),
            SliverPadding(
                ...
                child: Padding(
                  ...
                  child: PulseSegmented<String>(
                    ...
                    options: const [
                      ...
                      PulseSegmentedOption(value: 'In progress', label: 'In progress'),
                      PulseSegmentedOption(value: 'Resolved', label: 'Resolved'),
                      PulseSegmentedOption(value: 'Rejected', label: 'Rejected'),
                    ],
                  ),
                ),
              ),
              ...
              sliver: SliverToBoxAdapter(
                child: AsyncView<List>(
                  ...
                  builder: (context, allItems) {
                    final items = _filter == 'All' ? allItems : allItems.where((it) => '${(it as Map)['status']}' == _filter).toList();
                    if (items.isEmpty) return PulseEmptyState(illo: PulseIllo.noData, title: 'No complaints in "$_filter"');
                    return Column(
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    const Spacer(),
                                    PulsePill(label: status, tone: _statusTone(status), small: true),
                                  ],
                                ),
                                const SizedBox(height: 8),
                                Text('${it['title'] ?? ''}', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13.5, color: t.fg1)),
                                const SizedBox(height: 3),
                                Text('${it['description'] ?? ''}', style: TextStyle(fontSize: 12, color: t.fg3), maxLines: 2, overflow: TextOverflow.ellipsis),
                                const SizedBox(height: 8),
                                Text(
