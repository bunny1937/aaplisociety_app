    ...
    return AsyncView<List>(
      ...
      builder: (context, all) {
        ...
        final filtered = all.where((v) {
          ...
          final haystack = '${v['name']} ${v['vehicleNumber'] ?? ''}'.toLowerCase();
        ...
        return ListView(
          ...
          children: [
            Text('Visitor Log', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: t.fg1, letterSpacing: -0.3)),
            Text('Searchable history of every gate entry', style: TextStyle(fontSize: 12.5, color: t.fg4)),
            const SizedBox(height: 16),
            PulseSearchField(controller: _search, hint: 'Name, flat, vehicle...'),
            const SizedBox(height: 12),
            PulseSegmented<String>(
              options: _filters.map((f) => PulseSegmentedOption(value: f, label: f)).toList(),
              value: _filter,
              onChanged: (v) => setState(() => _filter = v),
            ),
            const SizedBox(height: 16),
            if (filtered.isEmpty)
              const PulseEmptyState(illo: PulseIllo.noData, title: 'No matching entries')
            else
              ...filtered.asMap().entries.map((e) {
                final v = e.value as Map;
                final id = v['_id'] as String;
                final status = '${v['status']}';
                final entryTime = DateTime.tryParse('${v['entryTime'] ?? v['createdAt']}');
                final timeLabel = entryTime == null ? '' : DateFormat('hh:mm a').format(entryTime.toLocal()).toLowerCase();
                ...
                return Padding(
                  ...
                  child: LedgerVisitorCard(
                    index: e.key,
                    name: '${v['name']}',
                    subtitle: '${v['purpose'] ?? ''} · $timeLabel${vehicle.isNotEmpty ? ' · $vehicle' : ''}',
                    status: status,
