...
class _VerifyTabState extends State<VerifyTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        Text('Verify Gate Pass', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: t.fg1, letterSpacing: -0.3)),
        Text('Validate a pre-approved visitor', style: TextStyle(fontSize: 12.5, color: t.fg4)),
        const SizedBox(height: 18),
        if (_result == null) ...[
          Row(
            children: [
              Expanded(child: _ModeChip(label: 'OTP', icon: Icons.dialpad_rounded, active: _mode == 0, onTap: () { Haptics.select(); setState(() => _mode = 0); })),
              const SizedBox(width: 10),
              Expanded(child: _ModeChip(label: 'QR token', icon: Icons.qr_code_rounded, active: _mode == 1, onTap: () { Haptics.select(); setState(() => _mode = 1); })),
            ],
          ),
          const SizedBox(height: 18),
          if (_mode == 0)
            ...
            TextField(
              ...
              controller: _qr,
              autofocus: true,
              decoration: const InputDecoration(hintText: 'Paste the scanned code'),
            ),
          const SizedBox(height: 18),
          PulseButton(label: 'Verify & grant entry', full: true, loading: _submitting, icon: Icons.shield_rounded, onTap: () => _verify(dio)),
          ...
          Center(
            child: Column(
              children: [
                ...
                Text('Verified visitor details will appear here', style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700, color: t.fg3)),
              ],
            ),
          ),
        ] else
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Entry granted', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: t.fg1)),
                    Text('Visitor admitted', style: TextStyle(fontSize: 12.5, color: t.fg4)),
                  ],
                ),
              ),
            ],
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(label, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: active ? t.brand : t.fg3)),
          ],
        ),
      ),
    );
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w700, color: t.fg1)),
        ],
      ),
    );
  }
