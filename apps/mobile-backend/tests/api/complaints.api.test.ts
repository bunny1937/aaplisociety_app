...
describe("POST /v1/complaints", () => {
  it("creates a complaint as Member with status Open and the member attached", async () => {
  ...
  it("an anonymous complaint has no memberId attached, in the response or in the DB", async () => {
    const societyId = randomObjectId()
    const memberId = randomObjectId()
    const token = bearerToken({ role: ROLES.MEMBER, societyId: String(societyId), memberId: String(memberId) })
    const res = await request(app).post("/v1/complaints").set(authHeader(token)).send({
    ...
    expect(res.status).toBe(201)
    expect(res.body).not.toHaveProperty("memberId")
...
describe("GET /v1/complaints", () => {
  it("a member sees only their own complaints, an admin sees all", async () => {
    ...
    const c1 = await Complaint.create({ societyId, memberId: member1, category: "noise", title: "T1", description: "D1 description long enough", status: "Open" })
    const c2 = await Complaint.create({ societyId, memberId: member2, category: "water", title: "T2", description: "D2 description long enough", status: "Open" })
...
describe("PATCH /v1/complaints/:id/status", () => {
  it("rejects Member with 403", async () => {
    const societyId = randomObjectId()
    const memberId = randomObjectId()
    const complaint = await Complaint.create({ societyId, memberId, category: "noise", title: "T", description: "Description long enough", status: "Open" })
    const token = bearerToken({ role: ROLES.MEMBER, societyId: String(societyId), memberId: String(memberId) })
